...
class ApiError extends Error {
  ...
  constructor(message: string, statusCode?: number, isRateLimit = false) {
    super(message);
    this.name = 'ApiError';
...
async function fetchWithErrorHandling(url: string): Promise<Response> {
  ...
  if (res.status === 429) {
    throw new ApiError('Rate limit exceeded. Please wait a moment and try again.', 429, true);
  }

  if (!res.ok) {
    const statusText = res.statusText || 'Unknown error';
    throw new ApiError(`API request failed: ${statusText} (${res.status})`, res.status);
  }
...
export async function fetchCategories(): Promise<CategoryOption[]> {
  const res = await fetchWithErrorHandling(`${BASE_URL}/categories.php`);
...
export async function searchRecipes(query: string, category?: string): Promise<Recipe[]> {
  ...
  if (query.trim()) {
    const res = await fetchWithErrorHandling(`${BASE_URL}/search.php?s=${encodeURIComponent(query.trim())}`);
    const data: SearchResponse = await res.json();
    recipes = (data.meals ?? []).map(mapMealToRecipe);
  } else if (category) {
    const res = await fetchWithErrorHandling(`${BASE_URL}/filter.php?c=${encodeURIComponent(category)}`);
    ...
    const detailResponses = await Promise.all(
      filteredMeals.map((m) => fetchWithErrorHandling(`${BASE_URL}/lookup.php?i=${m.idMeal}`))
    );
...
export async function fetchRecipeById(id: string): Promise<Recipe | null> {
  const res = await fetchWithErrorHandling(`${BASE_URL}/lookup.php?i=${id}`);
  const data: SearchResponse = await res.json();
